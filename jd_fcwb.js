...
!(async () => {
    ...
    if (author) {
        if (author.fcwbinviteCodeArr && author.fcwbinviterArr) {
            fcwbinviteCodeArr = [...fcwbinviteCodeArr]
            fcwbinviterArr = [...fcwbinviterArr]
        }
    }
    for (let i = 0; i < cookiesArr.length; i++) {
        if (!cookiesArr[i]) {
...
function wb(round, rowIdx, colIdx) {
    ...
    return new Promise((resolve) => {
        ...
        const nm = {
            url: `${JD_API_HOST}/?functionId=happyDigDo&body={"round":${fcwbroud},"rowIdx":${rowIdx},"colIdx":${colIdx},"linkId":"yCcpwTLIbY6pjaM42ACUVg"}&t=1635561607124&appid=activities_platform&client=H5&clientVersion=1.0.0`,
